...
**Your aviation world, personalized.**

An interactive, personalized aviation platform: pick your country and
the whole experience adapts — live flight tracking, daily aviation
challenges with XP and streaks, an aircraft encyclopedia, auto-ingested
aviation news, country-specific airports, and country/global
leaderboards.

This is a **real, runnable full-stack project** — not a mockup. Some
features (auth, XP, daily challenges, live map with demo mode, country
personalization) work immediately with zero configuration. Others
(real live flight data, real news sources) need you to add
credentials/URLs, which this README walks through.

**What changed in V4 vs V3:** country personalization (a first-visit
"Where are you flying from?" selector, generalized from the old
Argentina-only mode to any curated country), a from-scratch performance
rewrite of the live map (GeoJSON + clustering instead of one DOM marker
per aircraft), and a real `/profile` page. Nothing from V3 was removed —
`?argentina=1` still works everywhere it did before. See section 20 for
the full list of what's new, and FINAL_AUDIT.md for an honest
area-by-area status.
...
npm run db:seed:local           # loads starter aircraft/airports/news sources
npm run db:migrate:v4:local     # V4: adds country_code/airports_discovered columns
npm run build
npm run pages:dev               # serves the built site + functions locally, with D1
```

`pages:dev` runs at `http://localhost:8788` by default. In a second
...
**Important — local D1 persistence:** `wrangler d1 execute --local`
(used by `db:migrate:local` / `db:seed:local`) and `wrangler pages dev`
each run their own local Miniflare-backed SQLite instance. If they
don't agree on WHERE that local database file lives, `pages:dev` ends
up reading an empty, different database than the one migrations wrote
to — surfacing as `D1_ERROR: no such table: aircraft` even though the
migration/seed commands reported success. To prevent this, all three
scripts above explicitly pass the same flag:

```
--persist-to=.wrangler/state
```

Don't run any of these three commands with a different or missing
`--persist-to` value, and don't add a `preview_database_id` to
`wrangler.toml` unless it's a real database id (see the comment in
`wrangler.toml`) — either one can point `pages dev` at a different
local database than the one you just migrated/seeded.

If you ever get into a confusing local-DB state (tables missing, stale
seed data, etc.), the reliable reset is:

```bash
rm -rf .wrangler
npm run db:migrate:local
npm run db:seed:local
npm run db:migrate:v4:local
npm run pages:dev
```
...
> feature until that logic is added — see the "architecture in place"
> paragraph below for the shape it should take.
...
## 20. V4 — country personalization

V4's central idea ("Your aviation world, personalized.") is implemented
as follows:

**Fully working:**
- Country selector (`src/features/country/`) — first-visit "Where are
  you flying from?" modal with search + popular countries, localStorage
  persistence, and profile sync (`POST /api/profile/country`) for
  logged-in users. 12 curated countries (AR, US, GB, BR, CL, UY, ES,
  MX, FR, DE, AE, AU) — intentionally small per the spec, not a giant
  unordered list. Add more by editing `server/lib/countries.js` AND its
  frontend mirror `src/features/country/countries.js` together.
- **What's Flying Now performance rewrite** — this was the most
  important technical change. V3 created one real DOM element
  (`maplibregl.Marker`) per aircraft and rewrote its `innerHTML` on
  every ~20s poll; this did not scale. V4's `FlightMap.jsx` instead
  keeps every aircraft in a single GeoJSON source rendered via
  MapLibre's native `circle`/`symbol` layers (WebGL, not DOM), with
  built-in clustering (`cluster: true`) that automatically groups
  nearby aircraft at low zoom and expands as you zoom in. Updates are a
  single `source.setData(...)` call — no per-marker teardown/rebuild.
  Custom vector icons (a clean chevron, not an emoji) are canvas-
  rendered once per category and reused via `map.addImage`, rotated
  per-aircraft via a data-driven `icon-rotate` expression bound to
  `headingDeg` — no per-frame JS icon regeneration.
- GLOBAL / MY COUNTRY / NEAR ME modes on What's Flying Now (NEAR ME
  uses the browser Geolocation API with a graceful fallback to Global
  if denied/unavailable), plus a category breakdown (`counts` in the
  `/api/flights/live` response).
- `/api/flights/live`, `/api/airports`, `/api/leaderboard` all accept a
  new `?country=CODE` parameter. All pre-existing behavior (including
  the legacy `?argentina=1` alias) still works unchanged.
- A real `/profile` page (V3 never had one — it linked to the
  leaderboard with an explanatory tooltip instead). Shows avatar
  initial, username, country, level/XP with a progress bar, streak,
  and aircraft/airports/challenges counts — all from real profile
  columns, nothing fabricated in the UI.
- Daily Challenge gained a third working type, `AIRPORT_ICAO`
  ("What is the ICAO code of Ezeiza?"), which draws from whichever
  airports are seeded for a given country and — importantly — falls
  back to the full global airport pool if the selected country doesn't
  have enough seeded airports yet, so an under-seeded country can never
  break challenge generation (tested in `tests/v4-country.test.js`).
- Aviation News gained MY COUNTRY / GLOBAL / AIRLINES / AIRCRAFT /
  AIRPORTS / MILITARY / GENERAL AVIATION tabs (a `GENERAL_AVIATION`
  category rule was added to the news categorizer, which previously had
  no way to produce that category at all). "MY COUNTRY" is a pragmatic
  title/summary text-match against the country's name, documented as
  such in `functions/api/news/index.js` — it's a real, working filter,
  just not as precise as a dedicated per-article country column would be.
- Leaderboard gained a GLOBAL / MY COUNTRY / FRIENDS scope selector.
  FRIENDS is shown but disabled with an honest tooltip — there is no
  friends/social-graph feature or table in the schema, so rather than
  fake a "friends leaderboard" with unrelated data, it's clearly marked
  as not built yet.

**Known, honestly-documented limitation:** Daily Challenge is still
**one shared challenge per day for everyone**, not a distinct challenge
per country per day. `daily_challenges.challenge_date` has had a
column-level `UNIQUE` constraint since V3's `0001_init.sql`, and SQLite
cannot alter that constraint without rebuilding the table — a bigger,
riskier migration than this pass attempted. The new `country_code`
column on that table records which country (if any) that one shared
challenge happens to be about, and the `AIRPORT_ICAO` generator can
pull from any seeded country's airports, but there is no per-user or
per-country distinct daily challenge yet. Genuinely fixing this would
mean rebuilding `daily_challenges` without the single-column unique
constraint (e.g. a composite `UNIQUE(challenge_date, country_code)`
instead) — documented here as the concrete next step rather than
attempted in this pass.

**Migration:** V4 adds `database/migrations/0003_v4_country.sql`
(`country_code` on `profiles`/`airports`/`daily_challenges`,
`airports_discovered` on `profiles`). Apply it with `npm run
db:migrate:v4:local` (or `:remote`) **after** `0001_init.sql` and
`0002_seed.sql` — it's purely additive and safe to run on an existing
V3 database.

---

## 21. First Deployment Checklist
...
5. **Apply migrations** — `npm run db:migrate:remote` (schema) then
   `npm run db:seed:remote` (starter aircraft/airports/news sources),
   then `npm run db:migrate:v4:remote` (V4 country personalization
   columns). This actually creates the tables and fills in the data.
