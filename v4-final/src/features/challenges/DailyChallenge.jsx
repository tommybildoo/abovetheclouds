...
export default function DailyChallenge() {
  ...
  useEffect(() => {
    ...
    api(`/challenge/today${params}`).then((d) => setChallenge(d.challenge)).catch(() => setChallenge(null));
    // Only re-fetch on mount, not every time the country changes — the
    // country param is just a soft hint for the FIRST generation of the
    // day (see server/lib/challenges.js); it's not meant to re-roll an
    // already-generated challenge for the whole site every time someone
    // switches countries.
        ...
        <div className="dc__card reveal">
          ...
          {challenge && (
            <>
              ...
              <span className="dc__type">
                {challenge.challenge_type.replaceAll('_', ' ')}
                {challenge.country_code && challenge.country_code !== 'GLOBAL' ? ` · ${challenge.country_code}` : ''}
              </span>
