...
import { ARGENTINA_BBOX, getCountryBbox } from '../../../server/lib/geo.js';
...
export async function onRequestGet({ request, env }) {
  try {
    ...
    const countryParam = url.searchParams.get('country'); // V4: e.g. 'AR', 'US' — see server/lib/countries.js
    ...
    // Priority: explicit viewport bbox > ?country= (V4) > legacy
    // ?argentina=1 > none (global). A country not in the curated list
    // (getCountryBbox returns null) safely falls through to global
    // rather than erroring — see V4 spec section 12 ("no debe romper
    // el sistema si no hay datos suficientes").
    let bbox = null;
    if (minLat && maxLat && minLon && maxLon) {
      bbox = { minLat: +minLat, maxLat: +maxLat, minLon: +minLon, maxLon: +maxLon };
    } else if (countryParam) {
      bbox = getCountryBbox(countryParam) || (countryParam.toUpperCase() === 'AR' ? ARGENTINA_BBOX : null);
    } else if (argentina) {
      bbox = ARGENTINA_BBOX;
    }
    ...
    if (bbox && (countryParam || argentina)) {
      // Defensive fallback — bbox should already be respected by the
      // provider/cache query, but keep a JS-level filter as a safety
      // net in case flights came from an unfiltered cache read for any
      // reason. Generalized from the original Argentina-only check so
      // it applies equally to any V4 ?country= selection.
      filtered = filtered.filter((f) => f.latitude <= bbox.maxLat && f.latitude >= bbox.minLat && f.longitude <= bbox.maxLon && f.longitude >= bbox.minLon);
    }

    // Category breakdown — V4 "What's Flying" wants counts per category
    // alongside the total, not just a raw list.
    const counts = { total: filtered.length, PASSENGER: 0, CARGO: 0, MILITARY: 0, GENERAL_AVIATION: 0, UNKNOWN: 0 };
